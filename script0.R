...
## TEST ##
#set dates
sanFran_testSet$start_date = as.POSIXct(sanFran_testSet$start_date, format = "%m/%d/%Y %H:%M")
sanFran_testSet$end_date = as.POSIXct(sanFran_testSet$end_date, format = "%m/%d/%Y %H:%M")
#sanFran_trainSet$start_date = as.POSIXct(sanFran_testSet$start_date, format = "%Y-%m-%d %H:%M:%S")
#sanFran_trainSet$end_date = as.POSIXct(sanFran_testSet$end_date, format = "%Y-%m-%d %H:%M:%S")

#set duration
sanFran_testSet$duration_minutes = sanFran_testSet$duration_sec / 60
sanFran_testSet$duration_minutes = round(sanFran_testSet$duration_minutes, 2)

#add columns of day, month, year, weekend, hour
sanFran_testSet$year = format(sanFran_testSet$start_date,"%Y")
sanFran_testSet$Month = format(sanFran_testSet$start_date,"%B")
sanFran_testSet$Weekday = weekdays(sanFran_testSet$start_date)
sanFran_testSet$dayStatus = ifelse(sanFran_testSet$Weekday %in% c("Saturday","Sunday"),"Weekend","weekday")
sanFran_testSet$Hour = format(sanFran_testSet$start_date,"%H")
write.csv(sanFran_testSet, file = "C:\\Users\\User\\Dropbox\\IDC\\Courses\\BusinessAnalytics\\project\\bike\\dataSets\\san_francisco_data\\sanFran_test_extended.csv")
